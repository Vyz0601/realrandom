...
const createBaseQuery = (defaultOptions, useRQHook, overrideOptions) => {
  ...
  const getQueryOptions = (fetcherFn, variables) => {
    return {
      queryFn: context => fetcherFn(variables, context),
      queryKey: utils.getKey(defaultOptions.queryKey, variables)
    };
  };

  const getKey = variables => utils.getKey(defaultOptions.queryKey, variables);
  ...
  return Object.assign(utils.withMiddleware(useBaseHook, defaultOptions, 'queries'), {
